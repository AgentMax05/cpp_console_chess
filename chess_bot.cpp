...
std::vector<std::vector<int>> best_move(std::vector<std::vector<char>> board, std::string side) {
            ...
            for (int board_row = 0; board_row < board.size(); board_row++) {
                for (int board_column = 0; board_column < board[board_row].size(); board_column++) {
                    if (check_legal(board, current_piece, {piece_row, piece_column}, {board_row, board_column}, side) == true) {
                        if (check_check(board, current_piece, piece_row, piece_column, board_row, board_column, side) == true) {
    ...
    std::vector<double> move_values = value_moves(board, possible_moves, isLowercase, 8);

    double highest = 0.0;
    int move_index = -1;

    for (int i = 0; i < move_values.size(); i++) {
        if (move_values[i] > highest) {
            highest = move_values[i];
            move_index = i;
        }
    }

    std::cout << "Best value: " << highest;
    ...
    //     board[current[0][0]][current[0][1]] = current_piece;
    //     board[current[1][0]][current[1][1]] = previous_piece;
